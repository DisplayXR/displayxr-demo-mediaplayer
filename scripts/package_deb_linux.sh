...
#
# FFmpeg note (mediaplayer-specific): unlike the OpenXR loader, FFmpeg is NOT
# bundled. On Linux build_linux.sh resolves FFmpeg via pkg-config against the
# distro's system libav* shared libraries (libavformat-dev … in CI), so the
# binary dynamically links the standard-path libav*.so.N — no bundled copy, no
# rpath. compute_lib_depends() below picks those sonames up via `dpkg -S` and
# emits them as .deb Depends, so apt pulls the matching runtime packages
# (libavformat/…​/libavcodec) on install. (The slim static FFmpeg from
# scripts/build-ffmpeg-slim.sh is a Windows/MSVC installer concern only.)
#
...
# mediaplayer's CMake emits the binary directly under build/ (no build/linux/
# subdir — that's a modelviewer layout quirk), matching build_linux.sh.
...
# For mediaplayer this also captures the system FFmpeg + SDL3 runtime packages
# the binary dynamically links. --
compute_lib_depends() {
